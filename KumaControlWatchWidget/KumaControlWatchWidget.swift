...
@main
struct KumaControlWatchWidgetBundle: WidgetBundle {
    var body: some Widget {
        KumaControlWatchWidget()
    }
}

struct KumaControlWatchWidgetEntry: TimelineEntry {
    let date: Date
    let data: WidgetData?
}

struct KumaControlWatchWidgetProvider: TimelineProvider {
    ...
    func placeholder(in context: Context) -> KumaControlWatchWidgetEntry {
        KumaControlWatchWidgetEntry(
            date: now(),
            data: WidgetData(
                upCount: 5,
                totalCount: 5,
    ...
    func getSnapshot(in context: Context, completion: @escaping (KumaControlWatchWidgetEntry) -> Void) {
        completion(makeEntry())
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<KumaControlWatchWidgetEntry>) -> Void) {
        let entry = makeEntry()
        let nextUpdate = WidgetTimelineSupport.nextRefreshDate(from: now())
        completion(Timeline(entries: [entry], policy: .after(nextUpdate)))
    }

    private func makeEntry() -> KumaControlWatchWidgetEntry {
        KumaControlWatchWidgetEntry(
            date: now(),
...
struct KumaControlWatchWidgetEntryView: View {
    let entry: KumaControlWatchWidgetEntry
...
struct KumaControlWatchWidget: Widget {
    let kind = AppConstants.watchWidgetKind

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: KumaControlWatchWidgetProvider()) { entry in
            KumaControlWatchWidgetEntryView(entry: entry)
        }
