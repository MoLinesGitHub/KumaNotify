...
import IOKit.ps
@testable import KumaControl

final class KumaControlTests: XCTestCase {
    ...
    private func makeSettingsStore() -> (suiteName: String, store: SettingsStore) {
        let suiteName = "KumaControlTests.\(UUID().uuidString)"
    ...
    private func localizableCatalogContents() throws -> String {
        let repoRoot = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
        let catalogURL = repoRoot.appendingPathComponent("KumaControl/Resources/Localizable.xcstrings")
    ...
    func testMenuBarViewModelPublishesOfflineWidgetSnapshotAndReloadsWidget() async {
        ...
        let widgetSuiteName = "KumaControlTests.widget.\(UUID().uuidString)"
    ...
    func testMenuBarViewModelMarksAggregateStatusUnreachableWhenAnyServerFails() async {
        ...
        let widgetSuiteName = "KumaControlTests.widget.aggregate.\(UUID().uuidString)"
        let widgetDefaults = UserDefaults(suiteName: widgetSuiteName)!
        UserDefaults.standard.removePersistentDomain(forName: widgetSuiteName)
        defer { UserDefaults.standard.removePersistentDomain(forName: widgetSuiteName) }
    ...
    func testLocalizableCatalogContainsSystemSurfaceMetadata() throws {
        ...
        [
            ...
            "Allow notifications for Kuma Control in System Settings, then enable them again here.",
    ...
    func testDashboardViewModelBuildEmailReportEncodesBodyWithReservedCharacters() throws {
        ...
        XCTAssertEqual(subject, String(localized: "Kuma Control — Status Report"))
    ...
    func testWidgetDataClearRemovesPersistedSnapshot() {
        let suiteName = "KumaControlTests.widget.clear.\(UUID().uuidString)"
    ...
    func testKumaControlAppLaunchBehaviorRestoresMonitoringWhenConnectionExists() {
        let behavior = KumaControlApp.launchBehavior(
    ...
    func testKumaControlAppLaunchBehaviorShowsOnboardingOnlyWhenNoConnectionAndIncompleteSetup() {
        XCTAssertEqual(
            KumaControlApp.launchBehavior(
    ...
    func testKumaControlAppShouldPresentOnboardingRespectsUITestOverride() {
        XCTAssertTrue(KumaControlApp.shouldPresentOnboarding(
            showOnboarding: true,
            uiTestShowsOnboarding: false
        ))
        XCTAssertFalse(KumaControlApp.shouldPresentOnboarding(
    ...
    func testKumaControlAppShouldSeedUITestServerConnectionOnlyWhenStoreIsEmpty() {
        XCTAssertTrue(KumaControlApp.shouldSeedUITestServerConnection(true, existingConnectionCount: 0))
        XCTAssertFalse(KumaControlApp.shouldSeedUITestServerConnection(true, existingConnectionCount: 1))
        XCTAssertFalse(KumaControlApp.shouldSeedUITestServerConnection(false, existingConnectionCount: 0))
    }

    func testKumaControlAppClearSharedWidgetDataRemovesSnapshotAndReloads() {
        let suiteName = "KumaControlTests.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suiteName)!
        defer { UserDefaults.standard.removePersistentDomain(forName: suiteName) }
        ...
        var reloadCount = 0
        KumaControlApp.clearSharedWidgetData(
            defaults: defaults,
            reloadWidgets: { reloadCount += 1 }
        )
    ...
    func testProjectYAMLBundlesLocalizableCatalogIntoWidgetTarget() throws {
        ...
        XCTAssertTrue(project.contains("KumaControlWidget:"))
        XCTAssertTrue(project.contains("- path: KumaControl/Resources/Localizable.xcstrings"))
    }
